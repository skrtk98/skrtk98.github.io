...
:   位相空間 $`X,Y`$ の間の写像 $`f:X\to Y`$ に対して, 次が成り立つ.
    1. $`f`$ が連続単射であるならば, $`f`$ が位相的埋め込みであることと, 任意の開集合 $`U\subseteq X`$ に対して $`f(U)`$ が部分空間 $`f(X)`$ で開集合であることは同値である.
    2. $`f`$ が連続全射であるならば, $`f`$ が商写像であることと, 任意の部分集合 $`G\subseteq Y`$ に対して
        ```math
...
<u>proof.</u>
:   $`f`$ が連続単射であると仮定する.
    $`f`$ を終域 $`f(X)`$ に制限した写像 $`\widetilde f:X\to f(X)`$ は, 連続な全単射である.
    $`\widetilde f`$ が同相写像であることは, その逆写像が連続であることと同値である.
    逆写像が連続であることは, 任意の開集合 $`U\subseteq X`$ に対して $`\widetilde f(U)=f(U)`$ が $`f(X)`$ で開集合となることと同値である.
    したがって (1) が成り立つ.
    $`f`$ が連続全射であると仮定する.
    商写像の定義は, 任意の $`G\subseteq Y`$ について $`G`$ が $`Y`$ で開集合であることと $`f^{-1}(G)`$ が $`X`$ で開集合であることが同値であることを要求する.
    よって (2) は商写像の定義そのものである.
