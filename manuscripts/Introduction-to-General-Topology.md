...
Remark. **$`R_n`$ 記号の対応**
:   本稿では, 分離性の名称として対称的空間, 前正則空間, 弱 Urysohn 性, 正則性を用いる.
    文献で用いられる $`R_0`$, $`R_1`$, $`R_{1\frac12}`$, $`R_2`$ という記号は, それぞれこれらの性質に対応する.
    $`R_{1\frac12}`$ の記号と名称には文献による差があるため, 本稿では弱 Urysohn 性を主名称とする.

Proposition. **古典的な分離公理の対応**
:   現在の定義による空間について, 次の対応が成り立つ.
    1. $`\mathbf{T}_0`$ 空間であることと Kolmogorov 空間であることは同値である.
    2. $`\mathbf{T}_1`$ 空間であることと $`\mathbf{T}_0`$ かつ $`R_0`$ であることは同値である.
    3. Hausdorff 空間であることと $`\mathbf{T}_0`$ かつ $`R_1`$ であることは同値である.
    4. Urysohn 空間であることと $`\mathbf{T}_0`$ かつ $`R_{1\frac12}`$ であることは同値である.
...
<u>proof.</u>
:   第一の主張は $`\mathbf{T}_0`$ 空間と Kolmogorov 空間の定義が一致することから従う.
    第二から第四の主張は, $`\mathbf{T}_1=\mathbf{T}_0\land R_0`$, $`\mathbf{T}_2=\mathbf{T}_0\land R_1`$, $`\mathbf{T}_{2\frac12}=\mathbf{T}_0\land R_{1\frac12}`$ という対応から従う.
...
:   次が成り立つ.
    ...
    3. $`\mathbf{T}_1`$ 空間では, 完全正則性は任意の点とそれを含まない閉集合を函数で分離できる性質として特徴づけられる.
