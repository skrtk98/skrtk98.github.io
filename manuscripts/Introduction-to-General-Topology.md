...
Definition. **$`R_0`$, $`R_1`$, $`R_{1\frac12}`$ 公理**
:   位相空間 $`X`$ が次の条件を満たすとき, それぞれの分離公理を満たすという.
    1. $`R_0`$ 公理: 位相的に識別可能な任意の 2 点が分離される.
    2. $`R_1`$ 公理: 位相的に識別可能な任意の 2 点が近傍で分離される.
    3. $`R_{1\frac12}`$ 公理: 位相的に識別可能な任意の 2 点が閉近傍で分離される.
    4. $`R_2`$ 公理: 任意の点 $`x`$ と $`x`$ を含まない閉集合 $`F`$ が近傍で分離される.

Proposition. **古典的な分離公理の対応**
:   現在の定義による空間について, 次の対応が成り立つ.
    1. $`T_0`$ 空間であることと Kolmogorov 空間であることは同値である.
    2. $`T_1`$ 空間であることと $`T_0`$ かつ $`R_0`$ であることは同値である.
    3. Hausdorff 空間であることと $`T_0`$ かつ $`R_1`$ であることは同値である.
    4. Urysohn 空間であることと $`T_0`$ かつ $`R_{1\frac12}`$ であることは同値である.
    5. Vietoris 空間であることと正則かつ $`\mathbf{T}_0`$ であることは同値である.
    6. Tychonoff 空間であることと完全正則かつ $`\mathbf{T}_0`$ であることは同値である.
    7. Tietze 空間であることと正規かつ $`\mathbf{T}_1`$ であることは同値である.
    8. $`\mathbf{T}_5`$ 空間であることと遺伝的正規かつ $`\mathbf{T}_1`$ であることは同値である.
    9. $`\mathbf{T}_6`$ 空間であることと完全正規かつ $`\mathbf{T}_1`$ であることは同値である.
    10. 正則空間であることと $`R_2`$ 公理を満たすことは同値である.

<u>proof.</u>
:   第一の主張は $`T_0`$ 空間と Kolmogorov 空間の定義が一致することから従う.
    第二から第四の主張は, $`T_1=T_0\land R_0`$, $`T_2=T_0\land R_1`$, $`T_{2\frac12}=T_0\land R_{1\frac12}`$ という定義から従う.
    第五と第六の主張は, Vietoris 空間と Tychonoff 空間の定義から従う.
    第七から第九の主張も, Tietze 空間, $`\mathbf{T}_5`$ 空間, $`\mathbf{T}_6`$ 空間の定義から従う.
    第十の主張は, $`R_2`$ 公理と正則性の定義が一致することから従う.

Proposition. **函数分離と完全正則性**
:   次が成り立つ.
    1. 完全正則空間では, 点とその点を含まない閉集合は函数で分離される.
    2. 函数で分離される部分集合は近傍で分離される.
    3. $`T_1`$ 空間では, 完全正則性は任意の点とそれを含まない閉集合を函数で分離できる性質として特徴づけられる.

<u>proof.</u>
:   第一と第三の主張は完全正則性の定義と同値な表現である.
    第二の主張は分離概念の含意系列から従う.

Proposition. **函数分離と正規性**
:   $`T_1`$ 空間 $`X`$ に対して, 次は同値である.
    1. $`X`$ は正規空間である.
    2. 任意の互いに交わらない閉集合 $`F,E`$ は函数で分離される.

<u>proof.</u>
:   (1) を仮定する.
    Urysohn の補題により, 互いに交わらない閉集合 $`F,E`$ に対して $`f^{-1}(\{0\})=F`$ かつ $`f^{-1}(\{1\})=E`$ を満たす連続写像 $`f:X\to[0,1]`$ が存在する.
    よって $`F,E`$ は函数で分離される.
    (2) を仮定すると, 函数分離は近傍分離を含意するから, 任意の互いに交わらない閉集合は近傍で分離される.
    したがって $`X`$ は正規空間である.

Proposition. **完全正則性と正規性の接続**
:   $`T_1`$ 空間 $`X`$ が正規かつ完全正則であるとき, 任意の互いに交わらない閉集合 $`F,E`$ は函数で分離される.
    さらに, $`X`$ が正規かつすべての閉集合を $`G_\delta`$ 集合としてもつとき, 任意の互いに交わらない閉集合は函数でちょうど分離される.

<u>proof.</u>
:   第一の主張は Urysohn の補題から従う.
    第二の主張では, 各閉集合 $`F,E`$ を連続写像の零集合として表す.
    すなわち $`u^{-1}(\{0\})=F`$ かつ $`v^{-1}(\{0\})=E`$ を満たす連続写像 $`u,v:X\to[0,1]`$ をとる.
    $`F\cap E=\emptyset`$ だから $`u+v`$ は $`X`$ 上で正であり,
    ```math
    h=\frac{u}{u+v}
    ```
    とおくと $`h`$ は連続で $`h^{-1}(\{0\})=F`$ かつ $`h^{-1}(\{1\})=E`$ を満たす.
...
:   (1) を仮定し, 分離された部分集合 $`A,B\subseteq X`$ をとる.
    ...
    ここで必要なのは, 部分空間で得た分離を $`X`$ へ単に持ち上げることではなく, $`A,B`$ が分離されていることを使った標準的な分離集合補題である.
    この補題は, $`\overline A\cap B=A\cap\overline B=\emptyset`$ と $`S`$ 内の分離近傍から, $`X`$ 内の分離近傍を構成する.
