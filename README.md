...
プレビューワーはブラウザで現在開いているページを SSE で監視する。
原稿を保存すると、そのページに対応する HTML だけを再生成し、ブラウザを自動的に再読み込みする。
`manuscripts/contents/` の共通原稿を変更した場合は、その原稿を `@import` している表示中のページだけを再生成する。
