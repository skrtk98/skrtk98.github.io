...
async function page(title, body, depth = 0, toc = "", liveReload = false) {
  ...
  const reloadScript = liveReload
    ...
    : "";
  return `<!doctype html>\n<html lang="ja">\n<head>\n<meta charset="utf-8">\n<meta name="viewport" content="width=device-width, initial-scale=1">\n<title>${title}</title>\n<style>${katexCss.replaceAll("url(fonts/", `url(${fontPrefix}fonts/`)}${CSS}</style>\n</head>\n<body for="html-export"><div class="crossnote markdown-preview">${body}<div class="md-sidebar-toc">${toc}</div></div><a id="sidebar-toc-btn">≡</a><script>(function(){var sidebarTOCBtn=document.getElementById("sidebar-toc-btn");function scrollAnchor(){var target=location.hash?document.getElementById(decodeURIComponent(location.hash.slice(1))):null;if(!target){var headings=document.querySelectorAll(".markdown-preview h1,.markdown-preview h2,.markdown-preview h3,.markdown-preview h4,.markdown-preview h5,.markdown-preview h6");target=[...headings].sort(function(a,b){return Math.abs(a.getBoundingClientRect().top)-Math.abs(b.getBoundingClientRect().top);})[0];}if(!target)return null;return {element:target,top:target.getBoundingClientRect().top};}sidebarTOCBtn.addEventListener("click",function(event){event.stopPropagation();var anchor=scrollAnchor();if(document.body.hasAttribute("html-show-sidebar-toc")){document.body.removeAttribute("html-show-sidebar-toc");}else{document.body.setAttribute("html-show-sidebar-toc",true);}if(anchor){requestAnimationFrame(function(){var delta=anchor.element.getBoundingClientRect().top-anchor.top;if(delta)window.scrollTo(0,window.scrollY+delta);});}});})();</script>${reloadScript}</body>\n</html>\n`;
}
